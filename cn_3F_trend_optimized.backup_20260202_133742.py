...
#banpeaktab
        ...
        def plot_signal_data(self):
                ...
                try:
                        ...
                        # ========== delta_f 자동 조정 경고 플래그 ==========
                        delta_f_adjusted_warning_shown = False
                        ...
                        for i, file_name in enumerate(selected_files):
                                ...
                                if "Spectrum":
                                        ...
                                        # ✅ delta_f가 너무 작으면 자동 조정
                                        if sampling_rate / delta_f > N:
                                                # duration 기반 재계산
                                                if duration is not None:
                                                        try:
                                                                # 숫자만 추출
                                                                match = re.findall(r"[-+]?\d*\.\d+|\d+", str(duration))
                                                                if match:
                                                                        duration_val = float(match[0])
                                                                        if duration_val > 0:
                                                                                hz_value = round(
                                                                                        1 / duration_val + 0.01, 2)
                                                                                ...
                                                                                perf_logger.log_warning(
                                                                                        f"⚠️ {file_name}: delta_f 자동 조정 "
                                                                                        f"({old_delta_f:.2f} → {delta_f:.2f} Hz)"
                                                                                )

                                                                                # ✅ 첫 파일에서만 사용자에게 한 번만 알림
                                                                                if not delta_f_adjusted_warning_shown:
                                                                                        QMessageBox.warning(
                                                                                                None, "안내",
                                                                                                f"delta_f 값이 너무 작아 {delta_f:.2f} Hz로 자동 조정되었습니다.\n"
                                                                                                f"(원본: {old_delta_f:.2f} Hz)"
                                                                                        )
                                                                                        delta_f_adjusted_warning_shown = True
                                                        except Exception as e:
                                                                perf_logger.log_warning(
                                                                        f"⚠️ {file_name}: duration 파싱 실패")
                                                                delta_f = max(delta_f, delta_f_min)
                                                else:
                                                        delta_f = max(delta_f, delta_f_min)
                                        ...
                                        N_fft = max(int(sampling_rate / delta_f), MIN_FFT_LENGTH)
                                        if N_fft > N:
                                                scaled_data = np.pad(scaled_data, (0, N_fft - N), 'constant')
                                                N = N_fft
                                                perf_logger.log_info(f"ℹ️ {file_name}: 제로 패딩 적용 ({N_fft} 샘플)")
