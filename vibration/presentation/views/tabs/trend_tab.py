...
class TrendTabView(QWidget):
    ...
    def _save_original_limits(self):
        self._original_limits['trend'] = (self.trend_ax.get_xlim(), self.trend_ax.get_ylim())
    
    def _reset_zoom(self):
        if 'trend' in self._original_limits:
            xlim, ylim = self._original_limits['trend']
            self.trend_ax.set_xlim(xlim)
            self.trend_ax.set_ylim(ylim)
            self.trend_canvas.draw_idle()
    ...
    def _on_scroll(self, event):
        ...
        if 'trend' not in self._original_limits:
            self._save_original_limits()
        ...
        self.trend_ax.set_xlim(xdata - (xdata - xlim[0]) * scale_factor,
                               xdata + (xlim[1] - xdata) * scale_factor)
        self.trend_ax.set_ylim(ydata - (ydata - ylim[0]) * scale_factor,
