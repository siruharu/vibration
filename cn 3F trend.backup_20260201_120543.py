...
#banpeaktab
        ...
        def plot_signal_data(self):
                ...
                for i, file_name in enumerate(selected_files):
                        ...
                        if "Spectrum":
                                ...
                                try:
                                        ...
                                        duration = float(duration2)
                                        hz_value = round(1 / duration2, 2)  # 소수점 6자리까지 반올림
                                        min_res = sampling_rate / delta_f
                                        continue


                                frequency_array = f  # 모든 파일이 동일한 f를 가정
                                ...
                                self.ax.plot(f, np.round(ACF * np.abs(P), 4), label=file_name, color=color, linewidth=0.5)
                ...
                self.ax.set_position([0.049, 0.1, 0.82, 0.8])  # [left, bottom, width, height] 형식으로 설정
                self.ax.grid(True)
                ...
                if legends:
                        ...
                        #self.ax.legend(loc="upper left", bbox_to_anchor=(1, 1), fontsize=8, handles=legend_patches, labels=legend_labels)
